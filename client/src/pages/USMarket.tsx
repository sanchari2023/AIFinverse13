...
const availableStocks = [
  ...
  { symbol: "AIFG", name: "AIFinverse Global", sector: "Finance" },
];
...
// Get US Financial Year (Jan - Dec)
const getUSFinancialYear = (date: Date): string => {
...
// Determine LT (Long Term) or ST (Short Term) - US: >365 days = LT
...
const getChartLink = (symbol: string): string => {
      ...
      <table className="min-w-full">
        ...
        <tbody className="divide-y divide-gray-100">
          {holdings.map((holding) => {
            ...
            // Get chart link
            const chartLink = getChartLink(holding.symbol);
...
// Open Positions Table - FIXED: All values in one line, no line breaks
    ...
    <form onSubmit={handleSubmit} className="space-y-5">
      ...
      <div>
        ...
        <input
          ...
          placeholder="Enter price (e.g., 175.32 or 0.5)"
          className="w-full px-4 py-2.5 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent text-gray-900 text-base"
          style={{ color: '#111827' }}
        />
...
const USMarket: React.FC = () => {
  ...
  const [transactions, setTransactions] = useState<Transaction[]>(() => {
    ...
    const saved = localStorage.getItem('transactions_us');
    return saved ? JSON.parse(saved) : [];
  });
  
  useEffect(() => {
    if (!useDemoData) {
      localStorage.setItem('transactions_us', JSON.stringify(transactions));
    }
    ...
    sells.forEach(sell => {
      ...
      if (buyTransaction) {
        ...
        closedPositionsList.push({
          ...
          fy: getUSFinancialYear(sellDate),
  ...
  const handleAddTransaction = (transaction: Omit<Transaction, 'id'>) => {
    ...
    alert(`${transaction.action} transaction added for ${transaction.stockName} on ${transaction.exchange}`);
        ...
        <div className="relative z-10 max-w-full mx-auto px-4 sm:px-6 lg:px-8 py-8">
          ...
          <div className="mb-8">
            <h1 className="text-2xl font-bold text-white drop-shadow-lg flex items-center gap-2">
              <span className="text-3xl">🇺🇸</span> US Market - Portfolio Management
            </h1>
            <p className="text-sm text-white/80 drop-shadow mt-1">NYSE / NASDAQ | Currency: US Dollar ($) | Financial Year: Jan - Dec</p>
          </div>
